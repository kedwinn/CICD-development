...
pipeline {
    ...
    stages {
        stage('Git checkout') {
            steps {
                ...
                git branch: 'main', url: 'https://github.com/kedwinn/CICD-development.git'
            ...
            steps {
                withSonarQubeEnv('SonarQube') {
                    withCredentials([string(credentialsId: 'sonarqube-token', variable: 'SONAR_TOKEN')]) {
                        sh """
                 mvn sonar:sonar \
                -Dsonar.projectKey=app-cicdpipeline \
                -Dsonar.host.url=http://18.134.131.34:9000 \
                -Dsonar.login=59dedac9bf7b7dd3852d474a92d67ef0ee71ffb9
                    """
                    }
                }
            }
    ...
    post {
        always {
            ...
            slackSend channel: '#team-devops-venturaltd', color: COLOR_MAP[currentBuild.currentResult], message: "*${currentBuild.currentResult}:* Job ${env.JOB_NAME} build ${env.BUILD_NUMBER} \n More info at: ${env.BUILD_URL}"
        }
    }
}
